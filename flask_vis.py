...
with open("/home/verittest/verit-web/aws.txt", "r") as f:
    creds = f.read().split(",")
access_key = creds[0]
secret_key = creds[1]
...
print(f"{pickle_path}UniProtMappings_human.pkl")
        ...
        #If the query is a text box format with name contains/begins/ends
            ...
            if query_type=="dijkstra":
                ...
                if default_PR or string_type=="gene":
                    ...
                    #query_dict = {str(x)+str(random.randint(0, 1000000))*100: str(x)+"000"*100 for x in range(81)}
                    print("Before json")
            ...
            if query_type=="single":
                ...
                if string_type=="gene":
                    ...
                    query = {str(x)+str(random.randint(0, 1000000)): str(x)+"000" for x in range(81)}
...
def make_bfs_query(query_type):
    if request.method=="POST":
        print("post at make_bfs_query.")
        max_linkers=int(request.form["max_linkers"])
        qtype = request.form["qtype"]
        print("Redicreting to bfs_query_Result.")
